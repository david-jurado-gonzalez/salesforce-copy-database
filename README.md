...
## ✨ Características Clave
  ...
  * **Manejo de Subconsultas (Padre-Hijo):** Cuando se usan subconsultas, la herramienta "desenrolla" los datos JSON anidados de la Query API en archivos CSV separados para objetos padre e hijo. Para mantener la vinculación de la relación, se añade una columna artificial al CSV del objeto hijo con el patrón `NombreCampoRelacion` (ej. `AccountId`). Esta columna contiene el `Id` del registro padre de la organización de origen, siendo vital para el mapeo durante el despliegue. Además, se ha implementado una lógica robusta en [`src/core/sfdc-api.ts`](src/core/sfdc-api.ts) para corregir automáticamente los nombres de objetos plurales incorrectos en las relaciones hijo, utilizando un caché de SObjects para mapear a los nombres API singulares correctos, incluyendo heurísticas de singularización validadas contra los metadatos reales.
...
*   **Extracción de Datos Asistida (Modo Interactivo):** Configura y ejecuta extracciones de datos complejas, incluyendo la gestión de consultas SOQL y la selección de API, de forma guiada. Consulta la [Guía de Extraer Datos (Modo Interactivo)](projects/salesforce-copy-database/docs/user_guides/interactive_mode/extract_data.md) para más detalles.
  ...
  *   **Sugerencia de Consultas para Backups (Modo Interactivo):** La funcionalidad de sugerencia de consultas para backups ha sido significativamente mejorada:
      *   **Exclusión Inteligente de Campos y Objetos de Paquetes Gestionados:** Los campos y objetos con prefijos de namespace (ej. `cgcloud__`) se excluyen por defecto de las consultas principales y subconsultas para mantener la relevancia de los datos de negocio. Se han añadido opciones configurables (`includeNamespacedFields` o `includeNamespacedObjectsInSubqueries`) para permitir su inclusión si es necesario.
      *   **Priorización Avanzada de Subconsultas:** Se ha implementado una estrategia robusta para seleccionar las relaciones hijo más importantes cuando se aplica el límite de 20 tipos de hijos SOQL. La priorización se basa en:
          *   Accesibilidad y consultabilidad del objeto hijo (utilizando los métodos `isSObjectQueryable` y `isSObjectAccessible` en [`src/core/sfdc-api.ts`](src/core/sfdc-api.ts)).
          *   Prioridad a objetos personalizados (`__c`).
          *   Prioridad a relaciones `Master-Detail` sobre `Lookup`.
          *   Exclusión de relaciones de sistema comunes (lista negra configurable).
          *   Anulación o complemento mediante configuración de usuario (`includeRelationships`, `excludeRelationships`, `customOrder`).
          *   Desempate por orden alfabético.
      ...
      *   **Opción para Consultar Solo Objetos "Sueltos" (sin Subconsultas):** Se ha añadido una opción configurable (`includeChildRelationshipsInBackupQueries`) que, al establecerse en `false`, permite generar consultas que solo extraen los campos del objeto principal, sin incluir ninguna subconsulta. La información de las relaciones se seguirá extrayendo a través de la descripción completa del esquema para permitir la reconstrucción.
      *   **Exclusión por Defecto de Campos Técnicos:** Los campos técnicos como `CreatedDate`, `LastModifiedDate`, `CreatedById`, `LastModifiedById`, y `SystemModstamp` son ahora excluidos por defecto de la consulta principal para simplificar las consultas y enfocarse en los datos de negocio.
      *   **Configurabilidad:** Se mantiene la capacidad de incluir estos objetos con namespace y/o campos técnicos a través de opciones de configuración, permitiendo un control granular sobre la generación de consultas.
...
*   **Gestión de Consultas y Caché:**
    *   **Infraestructura de Gestión de Archivos:** Se ha implementado una infraestructura robusta en [`src/core/queryFileManager.ts`](src/core/queryFileManager.ts) para gestionar las consultas. Este módulo es responsable de guardar y cargar consultas de respaldo (`saveBackupQueries`, `loadBackupQueries`) y el historial de consultas (`saveQueryHistory`, `loadQueryHistory`). Las consultas se organizan de forma lógica en el sistema de archivos, utilizando el alias de la organización como clave, en la ruta `queries/<org_alias>/`.
    *   **Caché de Sugerencias de Consultas de Respaldo:** La funcionalidad de sugerencia de consultas para backups en [`src/interactive/backupQuerySuggester.ts`](src/interactive/backupQuerySuggester.ts) ahora aprovecha `queryFileManager` para guardar las consultas de respaldo generadas. Al sugerir una consulta, la herramienta verifica si ya existe una versión cacheada para la organización actual y ofrece al usuario la opción de utilizar esta versión guardada o generar una nueva, optimizando el tiempo y el uso de la API.
    *   **Historial de Consultas en Ejecución:** El comando de ejecución de queries ahora utiliza `queryFileManager` para persistir las consultas ejecutadas en un historial. Esto permite que, al sugerir una consulta, el comando pueda ofrecer opciones tanto de las consultas de respaldo cacheadas como de su propio historial de consultas, facilitando la reutilización y el acceso rápido a operaciones previas.
